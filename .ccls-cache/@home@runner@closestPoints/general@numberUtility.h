...
namespace randUtil {
  ...
  // do not touch this variable from outside this file
  bool initialized = false;

  // highly recommended to not directly call this function from outside this file
  void init() {
    if (initialized) {
      throw runtime_error("randUtil::init() has already been called before");
    } else {
      srand(time(nullptr));
      initialized = true;
    }
